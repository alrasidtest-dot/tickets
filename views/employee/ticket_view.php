<?php
/**
 * Employee — ticket detail: the ticket, its attachments, the comment timeline,
 * and the add-comment form (optional attachment).
 *
 * Vars:
 *   $ticket             joined ticket row (Ticket::findDetailById)
 *   $comments           Comment::findByTicket rows (author_name, created_at…)
 ...
 *   $commented          true right after a successful PRG comment add
 *   $isAdmin            true when the viewer is an admin (reassignment UI)
 *   $agents             active agents for the admin reassignment selector
 *   $reassigned         true right after a successful PRG reassignment
 *   $pageTitle          ticket number (used for <title>)
 *
...
// Signed-in user's name — used to align their own comments as chat bubbles.
$currentUserName = Auth::fullName();

// An admin reaching this page has no employee ticket list to go back to.
$backPage = !empty($isAdmin) ? 'dashboard' : 'ticket_my';
...
?>
                <div class="page-head">
                    ...
                    <a class="btn btn-outline btn-sm" href="<?php echo e(BASE_URL); ?>?page=<?php echo e($backPage); ?>">
                ...
                <?php endif; ?>
                <?php if (!empty($reassigned)): ?>
                    <div class="alert alert--success"><?php echo e(t('admin_reassigned')); ?></div>
                <?php endif; ?>
